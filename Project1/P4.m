...
DC=zeros(n-1,1);%Driving Command
pDC=zeros(5,1)
...
for  k = 1:(n-1) 
    ...
    %Estimate the most driving command
    for i=1:5
        pDC(i)=sum(w0(Z(:,k)==i))/sum(w0);% Calculate the probabilty of each state(driving command)
    end
    [~,ind]=max(pDC);
    DC(k)=ind;   
end
